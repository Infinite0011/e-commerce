...
namespace App\Http\Livewire\Components\Admin\Customers;
...
class Show extends Component
{
    ...
    {
        $user_ids = $this->customer->users()->pluck('users.id')->toArray();
        return User::whereIn('id', $user_ids)->paginate(
            perPage: 10,
    ...
    {
        // $this->customer->users()->detach($this->userIdToEdit);

        // $this->userIdToEdit = null;
        $user = User::where('id', $this->userToEdit['id'])->first();
    ...
    {
        return view('livewire.components.admin.customers.show')
