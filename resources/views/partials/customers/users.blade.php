<div>
  ...
  <x-hub::modal.dialog form="editUser" wire:model="userToEdit">
    <x-slot name="title">
      Edit user
    </x-slot>
    <x-slot name="content">
      <x-hub::alert level="warning">
        <x-hub::input.group for="email"
                            label="Email">
            <x-hub::input.text wire:model.defer="userEmail" :inital-value="$userToEdit ? $userToEdit['email'] : ''"/>
        </x-hub::input.group>
      </x-hub::alert>
    </x-slot>
    <x-slot name="footer">
      <x-hub::button type="button" wire:click.prevent="$set('userToEdit', null)" theme="gray">{{ __('adminhub::global.cancel') }}</x-hub::button>
      <x-hub::button type="submit">Confirm and save</x-hub::button>
    </x-slot>
  </x-hub::modal.dialog>
