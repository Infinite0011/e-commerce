...
namespace App\Http\Livewire\Components\Admin\Products\Variants;
...
class VariantShow extends Component
{
    ...
    public function mount() {
        ...
        $this->isBaseVariant = ($baseVariant->id == $this->variant->id);
    }
    ...
    protected function rules()
    {
        return array_merge(
            [
                ...
                'variant.discount' => 'required|numeric|min:0|max:100',
    ...
    public function updateDiscount() {
        $baseVariant = $this->product->variants()->first();
        $this->variant->prices[0]->price->value = $baseVariant->prices[0]->price->value * (100 - $this->variant->discount) / 100;
        $temp = $this->basePrices['USD'];
        $temp['price'] = $this->variant->prices[0]->price->value / 100;
        $this->basePrices->put('USD', $temp);
        $this->variant->prices[0]->save();
    }
        ...
        if ($this->isBaseVariant) {
            ...
            foreach($variants as $variant) {
                if ($variant->id != $this->variant->id) {
                    $variant->prices[0]->price->value = $baseVariant->prices[0]->price->value * (100 - $variant->discount) / 100;
                    $variant->prices[0]->save();
                }
            }
        }
    ...
    {
        return view('livewire.components.admin.products.variants.show')
